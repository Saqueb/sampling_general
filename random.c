...
int write_sampled(char *fname, long int seed, int output_flag, float **rra, float **vva, float vaa);
...
void main()
{
  ...
  for(i=0;i<Noutput;i++)
    {
      strcpy(file,"../N-body-threads/output.nbody_");
      sprintf(num,"%.3f",nz[i]);
      strcat(file,num);
...
//      rc = rz[i];
      ...
      strcpy(file,"outputs/sampled.nbody_");
      sprintf(num,"%.4f",nz[i]);
      strcat(file,num);
      
      write_sampled(file, seed, oflag, rra, vva, aa);
...
int write_sampled(char *fname, long int seed, int output_flag, float **rra, float **vva, float vaa)
{
  ...
  for(ii=0;ii<MM;ii++)
    {
      srand(12345+ii);
      jj = ii*8 + (long)round(7.*rand()/(RAND_MAX+1.0));
      fwrite(&rra[jj][0],sizeof(float),3,fp1);
    
      fwrite(&vva[jj][2],sizeof(float),1,fp1);
    }
  fwrite(&dummy,sizeof(dummy),1,fp1); 
